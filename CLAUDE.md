...
1. **Rate limit** — **not in this codebase**. A Vercel WAF rule ("Chat API Rate Limiting": path `/api/chat`, 30 requests / 60s per IP → 429) blocks at the edge, so an abusive client never reaches the function and costs no compute or model spend. An earlier version called `checkRateLimit()` from `@vercel/firewall`; that SDK is for limits needing application context (upstream uses it to exempt authenticated internal traffic), and since every turn here is public and anonymous it was a redundant round-trip. Note the limit is per IP, so a shelter or library behind one NAT shares the budget.
