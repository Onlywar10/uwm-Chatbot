...
export const env = createEnv({
	server: {
		...
		// Headless-render service config. RENDERER_URL defaults to
		// `${APP_URL}/api/render`; the chromium pack URL overrides the bundled
		// Sparticuz Chromium download.
		RENDERER_URL: z.string().optional(),
		// REQUIRED: the shared secret guarding POST /api/render. That endpoint renders
		// arbitrary URLs in headless Chromium, so it is an SSRF vector — it must fail
		// closed. Making this required means an unset token is a startup error rather
		// than a silently disabled auth check (which is what an .optional() here did).
		RENDERER_AUTH_TOKEN: z.string().min(1),
